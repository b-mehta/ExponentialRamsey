...
theorem artanh_partial_series_bound_aux {y : ℝ} (n : ℕ) (hy₁ : -1 < y) (hy₂ : y < 1) :
  ...
  field_simp [this, hy'.ne', hy'.ne, hy₂.ne, hy₂.ne', hy'', geom_sum_eq, sub_ne_zero_of_ne,
    hy''.symm]
  ring
...
theorem artanh_partial_series_upper_bound {x : ℝ} (h : |x| < 1) (n : ℕ) :
  ...
  have B : ∀ y ∈ Icc (-|x|) |x|, |deriv F y| ≤ |x| ^ (2 * n) / (1 - x ^ 2) :=
    ...
    exact
      div_le_div (pow_nonneg (sq_nonneg _) _) (pow_le_pow_of_le_left (sq_nonneg _) yx _)
...
theorem newbound {x : ℝ} (h : |x| < 1) (n : ℕ) :
    |2 * ∑ i in range n, x ^ (2 * i + 1) / (2 * i + 1) - log ((1 + x) / (1 - x))| ≤
      2 * |x| ^ (2 * n + 1) / (1 - x ^ 2) :=
  by
  ...
  have A : ∀ y ∈ Ioo (-1 : ℝ) 1, deriv F y = -2 * (y ^ 2) ^ n / (1 - y ^ 2) :=
    ...
    field_simp [F, this, hy'.ne', hy'.ne, hy.2.Ne, hy.2.ne', hy'', geom_sum_eq, sub_ne_zero_of_ne,
      hy''.symm]
    ring
  ...
  have B : ∀ y ∈ Icc (-|x|) |x|, |deriv F y| ≤ 2 * |x| ^ (2 * n) / (1 - x ^ 2) :=
    ...
    have yx : y ^ 2 ≤ x ^ 2 := by
      ...
      rwa [abs_le]
    refine'
      div_le_div (mul_nonneg zero_le_two (pow_nonneg (sq_nonneg _) _)) _ this (sub_le_sub_left yx _)
...
theorem abs_add_four (a b c d : ℝ) : |a + b + c + d| ≤ |a| + |b| + |c| + |d| :=
...
theorem log_seven_near : |log 7 - 5543595633008 / 2848844606571| < 6 / 10 ^ 24 :=
  ...
  rw [abs_mul, abs_mul, abs_mul, abs_neg, abs_one, abs_neg, abs_two, one_mul,
    abs_of_pos (show (0 : ℝ) < 6 by norm_num1),
    show (2 ^ 6 : ℝ) / (3 ^ 2 * 7) = 64 / 63 by norm_num1]
